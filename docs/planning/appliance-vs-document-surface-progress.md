...
- [x] **`pkg-home-appliance-pass`** — Align home supporting surfaces to appliance mode; keep diagram palette unchanged.
- [x] **`pkg-document-pass`** — Align non-home surfaces to document mode tokens while preserving current tone.
- [x] **`pkg-utility-enum`** — Add a small, enumerated, token-backed global utility set (if adopted).
- [x] **`pkg-cleanup-docs`** — Remove drift, codify guardrails, and finalize documentation.
...
- 2026-04-25: Completed **`pkg-home-appliance-pass`**: introduced appliance/document home-support tokens in `src/app.css` and migrated `HomeRouteTidePanels.svelte` supporting surfaces to token usage (`.home-panel` muted states, home empty-state card/action, landscape-hint text, and SVG `HomeMenuTrigger` hover fill/stroke/text). Diagram scene palette remains untouched.
- 2026-04-25: Completed **`pkg-document-pass`**: migrated remaining non-home literals to document-mode tokens by wiring `LocationTowns2.svelte` prompt/guidance/input/link colors to semantic variables and adding document/appliance aliases (`--text-document-emphasis`, `--text-document-secondary`, `--text-link-accent`, `--border-control`) in `src/app.css`; also tokenized global `.tide-clock` and `.clock-readout` text/fill. Verification: `npm run test -- --run src/ui/routeSurfaceMode.test.ts src/ui/routes/home/homeRouteInstallFlow.test.ts` passed; touched-file lints clean.
- 2026-04-25: Completed **`pkg-utility-enum`**: added a constrained token-backed utility catalogue in `src/app.css` (`u-pad-surface-sm`, `u-stack-sm`, `u-nav-link-list`) plus shared spacing/radius tokens (`--space-1..3`, `--radius-sm`), then adopted those utilities in both menu shells (`PrimaryNavMenu.svelte`, `HomeRouteTidePanels.svelte`) to remove duplicated link-list/layout styling without behavior changes. Verification: `npm run test -- --run src/ui/routeSurfaceMode.test.ts src/ui/routes/home/homeRouteInstallFlow.test.ts` passed; touched-file lints clean.
- 2026-04-25: Completed **`pkg-cleanup-docs`**: codified post-migration guardrails and intentional exceptions in `appliance-vs-document-surface-plan.md` (mode-first token usage, bounded utility policy, shared menu shell/content boundary, and diagram-palette protection), and marked definition-of-done checkboxes complete.
