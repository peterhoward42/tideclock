...
- [ ] Tracked files: nothing embarrassing or accidental (except `docs/planning/log.txt` — keep for now; remove manually later)
- [ ] `.env.example` for `VITE_TIDE_PROXY_BASE_URL` (`.env` stays gitignored)
- [ ] README: accurate, no duplicate blocks, clone → test → build path clear
- [ ] Clear out `docs/planning/` when finished (keep `log.txt` until you remove it manually)
- [ ] Docs for outsiders: normative `docs/specs/` and in-app Tide/Software Nerd only
- [ ] `tools/towns2/`: short note that this is location data tooling, not runtime app
