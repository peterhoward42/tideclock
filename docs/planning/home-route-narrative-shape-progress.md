...
- [x] **Phase 2** — Script narrative: grouping and skim-oriented structure in the home route module (now `home/HomeRoute.svelte`, with a thin `Home.svelte` entry after Phase 5)
...
- [x] **Phase 5** — Optional thin route shell / extra modules (only if still warranted)
...
- [x] Script skim order + section comments in `src/ui/routes/home/HomeRoute.svelte` (orchestration module; `Home.svelte` is the thin entry)
...
- [x] Thin router shell + shared props type → `src/ui/routes/Home.svelte`, `src/ui/routes/home/homeRouteProps.ts`
...
|------|--------|------------------|
| 2026-04-18 | Phase 5 | `Home.svelte` forwards `HomeRouteProps` into `home/HomeRoute.svelte`; shared contract in `home/homeRouteProps.ts`. |
