...
| **Phases touched** | Phases 1–2 |
| **Notes** | Phase 1: URL/query helpers in `homeRouteUrlQuery.ts`. Phase 2: `Home.svelte` script grouped (inputs → collaborator → state → derived → onMount → $effect → handlers) with skim comments. |
