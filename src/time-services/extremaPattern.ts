/**
 * extremaPattern.ts — Conservative detector for civil-day extrema that
...
export enum ExtremaPatternDetection {
  IsTypical = 'isTypical',
...
function toTimedExtremes(extremes: TimeOrderedTideExtrema): TimedTideExtreme[] {
  return extremes.map((extreme, index) => {
    ...
    if (Number.isNaN(timeMs)) {
      throw new Error(`classifyExtremaPattern: extremes[${index}] has invalid timeUtc`);
    }
    return {
      type: extreme.type,
      timeMs,
 ...
 */
export function classifyExtremaPattern(
  extremes: TimeOrderedTideExtrema,
): ExtremaPatternDetection {
  if (extremes.length <= 1) {
    return ExtremaPatternDetection.IsTypical;
  }

  const timedExtremes = toTimedExtremes(extremes);

  if (timedExtremes.length > 4) {
    return ExtremaPatternDetection.MoreThanFourExtrema;
  }

  if (hasNonAlternatingAdjacentTypes(timedExtremes)) {
    return ExtremaPatternDetection.NonAlternatingAdjacentTypes;
  }

  if (hasTightlyBunchedExtrema(timedExtremes)) {
    return ExtremaPatternDetection.TightlyBunchedAdjacentExtrema;
  }

  if (hasWeakHesitationSwing(timedExtremes)) {
    return ExtremaPatternDetection.WeakHesitationSwing;
  }
