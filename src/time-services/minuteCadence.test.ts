...
import { subscribeMinuteCadence } from './minuteCadence';

describe('subscribeMinuteCadence', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  it('fires immediately with current minute epoch by default', () => {
    ...
    vi.setSystemTime(t0);
    const ticks: number[] = [];
    subscribeMinuteCadence((e) => ticks.push(e), { now: () => t0 });
  ...
  it('skips immediate fire when fireImmediately is false', () => {
    ...
    vi.setSystemTime(t0);
    const ticks: number[] = [];
    subscribeMinuteCadence((e) => ticks.push(e), { now: () => Date.now(), fireImmediately: false });
  ...
  it('fires on the next minute boundary then each minute', () => {
    ...
    const ticks: number[] = [];
    const unsub = subscribeMinuteCadence((e) => ticks.push(e), { fireImmediately: true });
  ...
  it('unsubscribe clears pending timeout and interval', () => {
    ...
    vi.setSystemTime(start);
    const ticks: number[] = [];
    const unsub = subscribeMinuteCadence((e) => ticks.push(e), { fireImmediately: false });
