...
/** Query param name; value must match {@link DIAGRAM_DEV_PREVIEW_NO_MORE_TIDES_TODAY_ID}. */
export const DIAGRAM_DEV_PREVIEW_QUERY_PARAM = 'diagramPreview';

/** Catalog id / query value for the first dev preview scenario. */
export const DIAGRAM_DEV_PREVIEW_NO_MORE_TIDES_TODAY_ID = 'no-more-tides-today';
...
export function buildDiagramDevPreviewNoMoreTidesTodayClock(params: {
  ...
  if (extremes.length === 0) {
    throw new Error('buildDiagramDevPreviewNoMoreTidesTodayClock requires at least one tide extreme');
  }
...
/** True when `search` requests the no-more-tides-today preview (dev builds only). */
export function diagramDevPreviewNoMoreTidesTodayRequestedFromSearch(
  search: string,
): boolean {
  if (!import.meta.env.DEV) return false;
  try {
    const q = new URLSearchParams(search);
    return q.get(DIAGRAM_DEV_PREVIEW_QUERY_PARAM) === DIAGRAM_DEV_PREVIEW_NO_MORE_TIDES_TODAY_ID;
  } catch {
    return false;
  }
}
