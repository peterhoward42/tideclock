/**
 * diagramDevPreviewAtypicalTideDay.ts — Dev preview `atypical-tide-day` (see docs/planning/diagram-dev-preview-catalog.md).
 ...
 */
export function buildDiagramDevPreviewAtypicalTideDay(params: {
  readonly extremesAtLocation: TideExtremesAtLocation;
}): DiagramDevPreviewAtypicalTideDay {
  ...
  const detection = isAtypicalTideExtremaPattern(synthetic);
  if (detection === TideExtremaPatternDetection.IsTypical) {
    throw new Error(
      "buildDiagramDevPreviewAtypicalTideDay: synthetic fixture must be atypical (check schedule vs detector)",
    );
  }
