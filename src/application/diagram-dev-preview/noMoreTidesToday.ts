...
export type NoMoreTidesTodayClock =
  | {
 ...
 */
export function buildNoMoreTidesTodayClock(params: {
  readonly extremesAtLocation: TideExtremesAtLocation;
  readonly utcIsoToLocalCanonicalTime: UtcIsoToLocalCanonicalTime;
}): NoMoreTidesTodayClock {
  const { extremes } = params.extremesAtLocation;
  if (extremes.length === 0) {
    throw new Error(
      "buildNoMoreTidesTodayClock requires at least one tide extreme",
    );
  }
