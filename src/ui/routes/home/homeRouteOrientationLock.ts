 ...
 */
export async function requestHomeLandscapeOrientationLock(): Promise<void> {
 ...
 */
export function mountHomeRouteOrientationLock(root: HTMLElement): () => void {
  let attempted = false;
  const runtime = globalThis as typeof globalThis & {
  ...
  const requestOnceIfInside = (event: Event): void => {
    ...
    attempted = true;
    teardown();
    void requestHomeLandscapeOrientationLock();
  };
